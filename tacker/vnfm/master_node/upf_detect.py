...
class OpenStackAPI():
    ...
    def upf_detect(self):
        ...
        if upf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','upf','report')
            self.lock=1
        elif upf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','upf','report')
            self.lock=1
        elif upf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','upf','report')
            self.lock=1
