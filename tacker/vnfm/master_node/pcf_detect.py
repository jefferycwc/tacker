...
class OpenStackAPI():
    ...
    def get_pcf_status(self,instance_id):
        ...
        #print("Get pcf instance status: " + str(get_instance_status_response.status_code))
    ...
    def pcf_detect(self):
        ...
        if pcf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','pcf','report')
            self.lock=1
        elif pcf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','pcf','report')
            self.lock=1
        elif pcf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','pcf','report')
            self.lock=1
