...
class OpenStackAPI():
    ...
    def nssf_detect(self):
        ...
        if nssf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','nssf','report')
            self.lock=1
        elif nssf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','nssf','report')
            self.lock=1
        elif nssf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','nssf','report')
            self.lock=1
