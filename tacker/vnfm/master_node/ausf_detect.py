...
class OpenStackAPI():
    ...
    def get_ausf_status(self,instance_id):
        get_ausf_status_url = 'http://' + self.OPENSTACK_IP + '/compute/v2.1/servers/' + instance_id
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_instance_status_response = requests.get(get_ausf_status_url, headers=headers)
        #print("Get ausf instance status: " + str(get_instance_status_response.status_code))
    ...
    def ausf_detect(self):
        ...
        if ausf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','ausf','report')
            self.lock=1
        elif ausf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','ausf','report')
            self.lock=1
        elif ausf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','ausf','report')
            self.lock=1
