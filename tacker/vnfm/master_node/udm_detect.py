...
class OpenStackAPI():
    ...
    def get_udm_status(self,instance_id):
        ...
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_instance_status_response = requests.get(get_udm_status_url, headers=headers)
        #print("Get udm instance status: " + str(get_instance_status_response.status_code))
