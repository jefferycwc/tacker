...
class OpenStackAPI():
    ...
    def nrf_detect(self):
        ...
        if nrf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','nrf','report')
            self.lock=1
        elif nrf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','nrf','report')
            self.lock=1
        elif nrf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','nrf','report')
            self.lock=1
