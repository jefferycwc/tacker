...
class OpenStackAPI():
    ...
    def udr_detect(self):
        ...
        if udr_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','udr','report')
            self.lock=1
        elif udr_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','udr','report')
            self.lock=1
        elif udr_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','udr','report')
            self.lock=1
